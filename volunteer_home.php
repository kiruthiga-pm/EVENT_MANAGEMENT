...
$v_id = isset($_SESSION['v_id']) ? $_SESSION['v_id'] : "";
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div >
    ...
    if ($row) {
        $organiser_id = $row['o_id'];
        echo "<p>The organizer ID is $organiser_id</p>";
    } else {
        echo "No organizer found for the provided volunteer ID.";
    }
            ?>
...
while ($row = pg_fetch_assoc($result)) {
    ...
    echo "<button onclick=\"openPopuph('{$row['h_name']}', '{$row['date']}', '{$row['type']}', '{$row['place']}','{$row['capacity']}', '{$row['start_time']}', '{$row['end_time']}','{$row['image']}')\">View More</button>"; // Changed to button for consistency
    echo "</div>";
    echo "</div>";
}
echo "</div>";
