...
<div class="filter-bar">
    <div>
        <input type="text" id="searchInput" placeholder="Search Hackathons...">
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['registerform'])) {
    ...
    if (strtotime($date) < time()) {
        ...
        echo "window.location.href='hackathon_display.php'";
    } else {
        // Connect to your database

        // Check if participant is already registered for the event
        $query = "SELECT * FROM team WHERE t_id in(SELECT p_id from team_members where p_id=$p_id) AND prog_id=$h_id AND table_name='hackathons'";
  ...
  // Execute the SQL query
    ...
    if (!$result) {
        echo "<div>Error: Unable to retrieve hackathons from the database.</div>";
    } else {
        // Display the retrieved events
        while ($row = pg_fetch_assoc($result)) {
            echo "<div class='hackathon'>";
            echo "<div class='hackathon-details'>";
            echo "<img src='{$row['image']}' alt='hackathon Image' class='hackathon-image'>";
            echo "<div class='hackathoninfo'>";
            echo "<div class='hackathon-name'>" . $row['h_name'] . "</div>";
            echo "<div class='hackathon-date'>" . $row['date'] . "</div>";
            echo "<div class='hackathon-type'>" . $row['type'] . "</div>"; // Added event type
            echo "</div>"; // event-info
            echo "</div>"; // event-details
            echo "<div class='actions'>";
            echo "<button onclick=\"openPopup('{$row['h_name']}', '{$row['date']}', '{$row['type']}', '{$row['place']}','{$row['capacity']}', '{$row['start_time']}', '{$row['end_time']}','{$row['image']}')\">View More</button>"; // Changed to button for consistency
            echo "<form method='post' action='team_details.php'>";
            echo "<input type='hidden' name='tab_name' value='hackathons'>";
            echo "<input type='hidden' name='e_id' value='{$row['h_id']}'>";
...
<div class="popup" id="popup">
    <div class="popup-content">
        ...
        <img id="popup-image" src="" alt="hackathon Image" class="hackathon-image">
        <h3 id="popup-hackathon-name"></h3>
...
<script>
    function openPopup(name, date, type, place, capacity,starttime,endtime, image) {
        var popup = document.getElementById('popup');
        var popupEventName = document.getElementById('popup-hackathon-name');
