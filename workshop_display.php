...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['registerform'])) {
    ...
    if (strtotime($date) < time()) {
        echo "<script>alert('Error: workshop date has already passed.')</script>";
        echo "window.location.href='workshop_display.php'";
    } else {
        // Connect to your database

        // Check if participant is already registered for the event
        $query = "SELECT * FROM team WHERE t_id in(SELECT p_id from team_members where p_id=$p_id) AND prog_id=$w_id AND table_name='workshops'";
        $result = pg_query($dbconn, $query);
        if (pg_num_rows($result) > 0) {
            echo "<script>alert('Error: Participant is already registered for this workshop.')</script>";
        } else {
  ...
  // Execute the SQL query
    ...
    if (!$result) {
        ...
        while ($row = pg_fetch_assoc($result)) {
            ...
            echo "<input type='hidden' name='tab_name' value='workshops'>";
            echo "<input type='hidden' name='e_id' value='{$row['w_id']}'>";
