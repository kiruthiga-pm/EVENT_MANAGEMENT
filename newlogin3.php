...
<head>
    ...
    <style>
        body {
            ...
            background-image: url('https://i2.wp.com/files.123freevectors.com/wp-content/original/202634-light-color-simple-background.jpg?w=420'); /* Use the correct path to your background image */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['login'])) {
        ...
        $result1 = pg_query_params($databaseConnection, 'SELECT o_name, o_id FROM organiser WHERE o_name = $1 AND o_id = $2', array($name, $user_id)); 
        ...
        if ($numRows > 0) {
            ...
            $_SESSION['o_id'] = $user_id;
            // Redirect to participant_home.html
            header("Location: organiser_home.php");
            exit();
        } else {
            // User doesn't exist, display an error message
            echo '<script>alert("Invalid credentials. Please enter correct info."); window.location.href = "newlogin3.php";</script>';
            exit();
        }
    }
    else {
        // User doesn't exist, display an error message
        echo '<script>alert("Invalid credentials in org. Please enter correct info."); window.location.href = "newlogin3.php";</script>';
        exit();
    }
    } 
...
?>
    ...
    <div id="login-box">
        <h2 id="form-title">ORGANISER-LOGIN</h2>
        <form action="newlogin3.php" method="post">
