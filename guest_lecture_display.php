...
<div class="filter-bar">
    <div>
        <input type="text" id="searchInput" placeholder="Search Guestlectures...">
        <select id="filterType">
            <option value="gl_name">Name</option>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['registerform'])) {
    ...
    if (strtotime($date) < time()) {
        echo "<script>alert('Error:guestlecture date has already passed.')</script>";
        echo "window.location.href='guest_lecture_display.php'";
    } else {
        // Connect to your database

        // Check if participant is already registered for the event
        $query = "SELECT * FROM team WHERE t_id in(SELECT p_id from team_members where p_id=$p_id) AND prog_id=$gl_id AND table_name='guest_lectures'";
        $result = pg_query($dbconn, $query);
        if (pg_num_rows($result) > 0) {
            echo "<script>alert('Error: Participant is already registered for this guest lectures.')</script>";
        } else {
  ...
  // Execute the SQL query
    ...
    if (!$result) {
        echo "<div>Error: Unable to retrieve guestlectures from the database.</div>";
    } else {
        // Display the retrieved events
        while ($row = pg_fetch_assoc($result)) {
            echo "<div class='guestlecture'>";
            echo "<div class='guestlecture-details'>";
            echo "<img src='{$row['image']}' alt='guestlecture Image' class='guestlecture-image'>";
            echo "<div class='guestlectureinfo'>";
            echo "<div class='guestlecture-name'>" . $row['gl_name'] . "</div>";
            echo "<div class='guestlecture-date'>" . $row['date'] . "</div>";
            echo "<div class='guestlecture-type'>" . $row['type'] . "</div>"; // Added event type
            echo "</div>"; // event-info
            echo "</div>"; // event-details
            echo "<div class='actions'>";
            echo "<button onclick=\"openPopup('{$row['gl_name']}', '{$row['date']}', '{$row['type']}', '{$row['place']}', '{$row['speaker']}','{$row['capacity']}', '{$row['start_time']}', '{$row['end_time']}','{$row['image']}')\">View More</button>"; // Changed to button for consistency
            echo "<form method='post' action='team_details.php'>";
            echo "<input type='hidden' name='tab_name' value='guest_lectures'>";
            echo "<input type='hidden' name='e_id' value='{$row['gl_id']}'>";
...
<div class="popup" id="popup">
    <div class="popup-content">
        ...
        <img id="popup-image" src="" alt="guestlecture Image" class="guestlecture-image">
        <h3 id="popup-guestlecture-name"></h3>
        ...
        <p><strong>Speaker:</strong> <span id="popup-speaker"></span></p>
...
<script>
    function openPopup(name, date, type, place,speaker, capacity,starttime,endtime, image) {
        var popup = document.getElementById('popup');
        var popupEventName = document.getElementById('popup-guestlecture-name');
        ...
        var popupspeaker = document.getElementById('popup-speaker');
