...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['login'])) {
        ...
        $result1 = pg_query_params($databaseConnection, 'SELECT p_name p_id FROM participants WHERE p_name = $1 AND p_id = $2 ', array($name, $user_id));
        ...
        if ($numRows > 0) {
            ...
            header("Location: participant_home.html");
            exit();
        }
        else {
            // User doesn't exist, display an error message
            echo '<script>alert("Invalid credentials. Please sign up first."); window.location.href = "newlogin1.php";</script>';
...
?>
    ...
    <div id="login-box">
        <h2 id="form-title">PARTICIPANT-LOGIN</h2>
